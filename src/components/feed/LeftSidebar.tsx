...
const exploreItems = [
  { label: "Learning", icon: "play", isNew: true },
  { label: "Insights", icon: "chart" },
  { label: "Find friends", icon: "friends" },
  { label: "Bookmarks", icon: "bookmark" },
  { label: "Group", icon: "group" },
  { label: "Gaming", icon: "gaming", isNew: true },
...
const events = [
  { title: "No more terrorism no more cry", date: "10", month: "Jul", going: 17, img: "/assets/images/feed_event1.png" },
  { title: "No more terrorism no more cry", date: "10", month: "Jul", going: 17, img: "/assets/images/feed_event1.png" },
];

function ExploreIcon({ icon }: { icon: string }) {
  switch (icon) {
    case "play":
      return (
    ...
    case "save":
      return (
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 22 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" /><polyline points="17 21 17 13 7 13 7 21" /><polyline points="7 3 7 8 15 8" /></svg>
      );
    default:
      return null;
  }
    ...
    <aside className="hidden flex-col overflow-auto pt-[18px] lg:flex" style={{ height: "calc(100vh - 70px)" }}>
        ...
        <ul>
          {exploreItems.map((item) => (
            <li key={item.label} className="relative mb-6 flex items-center justify-between">
              ...
              {item.isNew && (
                <span className="flex h-6 w-9 items-center justify-center rounded-lg border-2 border-[var(--card-bg)] bg-accent text-[13px] font-normal text-white">
                  New
                </span>
              )}
            </li>
      ...
      <div className="mb-4 rounded-sm bg-[var(--card-bg)] p-6 pb-1.5 transition-all duration-200">
        ...
        {suggestedPeople.map((person) => (
          <div key={person.name} className="mb-6 flex flex-wrap items-center justify-between">
