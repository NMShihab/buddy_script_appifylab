    ...
    <aside className="hidden flex-col overflow-auto pt-[18px] lg:flex" style={{ height: "calc(100vh - 70px)" }}>
      ...
      <div className="mb-4 rounded-sm bg-[var(--card-bg)] p-6 transition-all duration-200">
        <div className="mb-6 flex items-center justify-between">
          <h4 className="text-lg font-medium text-text-heading dark:text-white">You Might Like</h4>
          <span className="cursor-pointer text-xs font-medium text-primary">See All</span>
        </div>
        <hr className="mb-4 border-border-input dark:border-white/10" />
        {youMightLike.map((person) => (
          <div key={person.name}>
            ...
            <div className="flex gap-2">
              <button className="flex-1 rounded-[2px] border border-[#DCDFE4] py-[7px] text-xs font-medium text-[#959EAE] transition-colors hover:border-primary hover:text-primary dark:border-white/20 dark:text-white/50">
                Ignore
              </button>
      ...
      <div className="mb-4 rounded-sm bg-[var(--card-bg)] p-6 pb-1.5 transition-all duration-200">
        <div className="mb-6 flex items-center justify-between">
          <h4 className="text-lg font-medium text-text-heading dark:text-white">Your Friends</h4>
          <span className="cursor-pointer text-xs font-medium text-primary">See All</span>
        </div>
        {/* Search */}
        <div className="relative mb-4">
          ...
          <input
            type="search"
            placeholder="Search..."
            className="h-[38px] w-full rounded-sm border border-border-input bg-[var(--input-bg)] pl-10 pr-3 text-sm text-text-body outline-none placeholder:text-text-muted focus:border-primary dark:text-white"
          />
        </div>
        {/* Friends list */}
        <div className="max-h-[400px] overflow-auto">
          {friends.map((friend, i) => (
            <div
              key={i}
              className={`mb-4 flex cursor-pointer items-center justify-between rounded-sm p-2 transition-colors hover:bg-surface-input dark:hover:bg-dark-secondary ${!friend.online ? "opacity-60" : ""}`}
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-medium text-text-heading dark:text-white">{friend.name}</h4>
                  <p className="text-xs text-text-muted dark:text-white/60">{friend.role}</p>
                </div>
              </div>
              <div className="flex-shrink-0">
                {friend.online ? (
                  ...
                  <span className="text-[11px] text-text-muted dark:text-white/40">{friend.lastSeen}</span>
                )}
              </div>
            </div>
          ))}
